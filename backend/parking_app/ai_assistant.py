...
class ParkingQueryAssistant:
    ...
    def answer_query(self, user, query_text):
        """
        Answer a natural language query about parking.
        E.g., "Why was I assigned this slot?" "Can I get a closer slot?" "What are my options?"
        """
        ...
        elif any(word in query_lower for word in ['change', 'swap', 'different', 'closer', 'better']):
            if current_assignment:
        ...
        elif any(word in query_lower for word in ['subscription', 'upgrade', 'premium', 'vip']):
            return {
                'query_type': 'subscription',
                'response': "Upgrading your subscription gives you better parking benefits: "
                           "Premium tier offers priority access, VIP tier offers exclusive reserved spots. "
                           "Visit the Subscription page to upgrade.",
            }
        
        elif any(word in query_lower for word in ['how', 'work', 'algorithm', 'assign', 'system']):
            return {
                'query_type': 'system_info',
                'response': "SpotLight uses an intelligent algorithm that considers: "
                           "1) Your schedule and building location, "
                           "2) Parking availability during your time, "
                           "3) Your subscription tier for priority benefits, "
                           "4) Distance optimization to minimize walking distance. "
                           "The system automatically adjusts assignments when availability changes.",
            }
